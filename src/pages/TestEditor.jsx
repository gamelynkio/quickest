      ...
      <div style={{ display: "flex", alignItems: "center", gap: "3px", flexWrap: "wrap" }}>
        ...
        {SPECIAL_CHARS[activeLang].chars.map((char, i) => (
          <button key={i} type="button" onClick={() => insertChar(char)}
            style={{ padding: "2px 5px", minWidth: "22px", fontSize: "13px", background: "#fff", color: "#0f172a", border: "1px solid #e2e8f0", borderRadius: "4px", cursor: "pointer", fontFamily: "inherit", lineHeight: 1.4 }}>
            {char}
          </button>
        ))}
...
function TaskEditor({ task, tIdx, onUpdate, onRemove, onAddQuestion, onUpdateQuestion, onRemoveQuestion, onMoveUp, onMoveDown }) {
  ...
  localRef.current = { localTitle, localTaskText };
  useEffect(() => { return () => { onUpdate("taskTitle", localRef.current.localTitle); onUpdate("taskText", localRef.current.localTaskText); }; }, []);

  return (
    <div style={{ background: "linear-gradient(135deg, #1e3a5f, #2563a8)", borderRadius: "14px", padding: "20px 24px", marginBottom: "16px", color: "#fff" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
            <button onClick={onMoveUp} disabled={!onMoveUp}
              style={{ background: "none", border: "none", cursor: onMoveUp ? "pointer" : "default", color: onMoveUp ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.2)", fontSize: "12px", padding: 0 }}>▲</button>
            <button onClick={onMoveDown} disabled={!onMoveDown}
              style={{ background: "none", border: "none", cursor: onMoveDown ? "pointer" : "default", color: onMoveDown ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.2)", fontSize: "12px", padding: 0 }}>▼</button>
          </div>
          <span style={{ fontSize: "16px" }}>📋</span>
          <span style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.9)", letterSpacing: "0.5px" }}>AUFGABE {tIdx + 1}</span>
        </div>
      ...
      <div style={{ marginBottom: "12px" }}>
        ...
        <input ref={titleRef} value={localTitle} onChange={e => setLocalTitle(e.target.value)} onBlur={() => onUpdate("taskTitle", localTitle)}
          placeholder="z.B. Vocabulario – Unidad 4"
      ...
      <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: "10px", padding: "14px" }}>
        ...
        <div style={{ marginTop: (task.questions || []).length > 0 ? "10px" : "0" }}>
          <div style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.5)", letterSpacing: "0.5px", marginBottom: "6px" }}>UNTERAUFGABE HINZUFÜGEN</div>
              ...
              {localBlanks.map((blank, bi) => (
                <div key={bi} style={{ marginBottom: "6px" }}>
                  <div style={{ fontSize: "11px", color: "#374151", marginBottom: "3px" }}>
                    ...
                    Alternativen zu: <strong>{blank.solution || "–"}</strong>
                  </div>
...
export default function TestEditor({ navigate, onLogout, currentUser, editingTest }) {
  ...
  const addTask = () => setQuestions(prev => [...prev, newTask()]);
  const updateTask = (taskId, field, value) => setQuestions(prev => prev.map(t => t.id === taskId ? { ...t, [field]: value } : t));
  const removeTask = (taskId) => setQuestions(prev => prev.filter(t => t.id !== taskId));
  const addTaskQuestion = (taskId, type) => setQuestions(prev => prev.map(t => t.id === taskId ? { ...t, questions: [...(t.questions || []), newTaskQuestion(type)] } : t));
  const updateTaskQuestion = (taskId, qId, field, value) => setQuestions(prev => prev.map(t => t.id === taskId ? { ...t, questions: (t.questions || []).map(tq => tq.id === qId ? { ...tq, [field]: value } : tq) } : t));
  const removeTaskQuestion = (taskId, qId) => setQuestions(prev => prev.map(t => t.id === taskId ? { ...t, questions: (t.questions || []).filter(tq => tq.id !== qId) } : t));
      ...
      <div style={{ padding: "32px", maxWidth: "860px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", border: "1px solid #e2e8f0", marginBottom: "20px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "16px" }}>
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="z.B. Vocabulario – Unidad 4"
        ...
        {questions.map((task, index) => (
          <TaskEditor key={task.id} task={task} tIdx={index}
            ...
            onAddQuestion={(type) => addTaskQuestion(task.id, type)}
            onUpdateQuestion={(qId, field, val) => updateTaskQuestion(task.id, qId, field, val)}
            onRemoveQuestion={(qId) => removeTaskQuestion(task.id, qId)}
            onMoveUp={index > 0 ? () => moveTask(index, -1) : null}
            onMoveDown={index < questions.length - 1 ? () => moveTask(index, 1) : null}
          />
        ))}

        <button onClick={addTask} style={{ width: "100%", padding: "14px", border: "2px dashed #c7d2fe", borderRadius: "14px", background: "#fff", color: "#4f46e5", fontSize: "14px", fontWeight: 600, cursor: "pointer", marginBottom: "12px" }}
