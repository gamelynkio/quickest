...
export default function StudentTestView({ currentUser, assignment: assignmentProp, onFinish }) {
  ...
  const [submissionId, setSubmissionId] = useState(null); // als State für zuverlässiges Polling
  ...
  const serverOffsetRef = useRef(0); // Differenz zwischen Serverzeit und Date.now()
  ...
  useEffect(() => {
    const init = async () => {
      // Serverzeit-Offset berechnen damit alle Geräte dieselbe Zeit haben
      try {
        const t0 = Date.now();
      ...
      } catch (_) { serverOffsetRef.current = 0; }
  ...
  const fetchAssignment = async (preloaded = null) => {
    ...
    if (data) {
      ...
      if (data.timing_mode === "lobby" && !data.lobby_started_at) {
        ...
        const qtStudentJoin = JSON.parse(sessionStorage.getItem("qt_student") || "{}");
        await supabase.rpc("lobby_action", { _username: qtStudentJoin.username||"", _pin: qtStudentJoin.pin||"", _assignment_id: data.id, _action: "join" });
  ...
  useEffect(() => {
    ...
    const heartbeat = setInterval(async () => {
      ...
      await supabase.rpc("lobby_action", { _username: qtStudent.username||"", _pin: qtStudent.pin||"", _assignment_id: assignmentId, _action: "heartbeat" });
    ...
    const cleanup = async () => { const qtStudent = JSON.parse(sessionStorage.getItem("qt_student") || "{}");
      await supabase.rpc("lobby_action", { _username: qtStudent.username||"", _pin: qtStudent.pin||"", _assignment_id: assignmentId, _action: "leave" }); };
  ...
  // === POLL: Note + Freigabe alle 3 Sek. prüfen (läuft solange submissionId bekannt) ===
  useEffect(() => {
    if (!submissionId) return;
    // Sofort einmal abfragen
    supabase.from("submissions").select("released, grade, score, total_points").eq("id", submissionId).single()
      .then(({ data }) => { if (data) setSubmissionResult(data); });
    const poll = setInterval(async () => {
  ...
  // === POLL: Warten auf neuen Test wenn kein Assignment aktiv (alle 5 Sek.) ===
  useEffect(() => {
    if (assignment || loading) return; // Läuft nur wenn kein Test aktiv
    const poll = setInterval(async () => {
      const { data } = await supabase
        .from("assignments")
        .select("id")
      ...
      if (data.lobby_started_at) {
        ...
        if (startTime <= serverNow()) {
          ...
          if (data.lobby_end_at) {
            const remaining = Math.max(0, Math.round((new Date(data.lobby_end_at).getTime() - serverNow()) / 1000));
            setTimeLeft(remaining);
          } else {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (asgn.timing_mode === "lobby" && asgn.lobby_end_at) {
        ...
        const remaining = Math.max(0, Math.round((new Date(asgn.lobby_end_at).getTime() - serverNow()) / 1000));
        setTimeLeft(remaining);
  ...
  const handleSubmit = async () => {
    ...
    const qtStudent = JSON.parse(sessionStorage.getItem("qt_student") || "{}");
      await supabase.rpc("lobby_action", { _username: qtStudent.username||"", _pin: qtStudent.pin||"", _assignment_id: assignment.id, _action: "leave" });
    setSubmitted(true);
